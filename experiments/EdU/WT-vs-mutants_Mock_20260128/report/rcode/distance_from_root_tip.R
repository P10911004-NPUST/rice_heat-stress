...
df1 <- df0 %>% 
    dplyr::mutate(
        is_outlier = Grubbs_test(distance_from_root_tip_um),
        .by = "genotype"
    ) %>% 
    dplyr::filter(!is_outlier) %>%
    dplyr::slice_max(distance_from_root_tip_um, n = 6, by = "genotype")

out <- oneway_test(df1, distance_from_root_tip_um ~ genotype)
out$tests
cld <- out$cld
cld <- cld %>% 
    dplyr::mutate(
...
ggplot(df1, aes(genotype, distance_from_root_tip_um, color = genotype)) +
    theme_classic() +
    labs(
        y = "Distance from root tip (<i>&micro;</i>m)"
    ) +
    geom_boxplot(
        fill = "transparent",
        size = 1,
    ...
    scale_y_continuous(limits = c(450, 850), breaks = seq(450, 850, 100)) +
    scale_color_manual(
