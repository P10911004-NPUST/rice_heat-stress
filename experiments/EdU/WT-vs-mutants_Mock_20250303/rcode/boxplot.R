...
csv_list <- list.files("../", pattern = "\\.csv", full.names = TRUE)
...
df1 <- rawdata %>% 
    dplyr::filter(!str_detect(img_name, "cancel")) %>% 
    dplyr::filter(str_detect(img_name, "fluo")) %>% 
    dplyr::mutate(
        genotype = gsub("(.*)_(.*)_(.*)_(.*)_(.*).czi", "\\2", img_name)
    ) %>% 
    dplyr::mutate(
        genotype = factor(genotype, levels = c("WT", "M07", "M08", "M15"))
    )
...
# EdU avg intensity ====
####################################################################################################
stats_res <- oneway_test(df1, avg_intensity ~ genotype)
stats_res$tests
stats_res <- stats_res$result
stats_res

ggplot(df1, aes(genotype, avg_intensity, color = genotype)) +
    labs(
        subtitle = "EdU signal",
        y = "Average intensity (DN)"
    ) +
    ...
    geom_text(
        inherit.aes = FALSE,
        data = stats_res,
        mapping = aes(GROUPS, MAX + 15, label = CLD),
        size = 8
    ) +
    scale_color_manual(values = c("#fb8500", "#8ecae6", "#219ebc", "#023047")) +
    theme(
