...
df0 <- rawdata %>% 
    ...
    dplyr::mutate(
        ...
        genotype = case_when(genotype == "XVE-OsRGF1-28-3" ~ "OE",
                             TRUE ~ genotype),
        group = paste(genotype, DMSO_ppm, Estradiol_uM, sep = "_")
    ) %>% 
    dplyr::mutate(
...
df0$group <- factor(
    df0$group, 
    levels = c("OE_0_0", "OE_100_0", "OE_100_0.01", "OE_100_0.1",
               "OE_100_1", "OE_100_2", "OE_100_5", "OE_100_10")
)
...
out$tests
cld <- out$cld
print(cld[match(levels(df0$group), cld$GROUP), ])
...
ggplot(df0, aes(group, nbt_total, color = group)) +
    theme_classic() +
    labs(
    ...
    hline_grob(1 - 0.3, 4 + 0.3, 0 - 1) +
    hline_grob(5 - 0.3, 8 + 0.3, 0 - 1) +
    theme(
