...
ggplot(df0, aes(grp, val, color = genotype)) +
    ...
    hline_grob(1 - 0.3, 2 + 0.3, 30 - 2.5) +
    hline_grob(3 - 0.3, 4 + 0.3, 30 - 2.5) +
    hline_grob(5 - 0.3, 6 + 0.3, 30 - 2.5) +
    theme(
...
ggsave(
    create.dir = TRUE,
    filename = "root-length_peptide-recover.jpg",
    path = "./figure/",
    device = "jpeg",
    dpi = 660,
    units = "cm",
...
df0 %>% 
    dplyr::arrange(grp) %>% 
    dplyr::select(genotype, OsRGF1_pM, root_length) %>% 
    write.csv("./figure/root-length_peptide-recover.csv", row.names = FALSE)
