...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
grp_lvl <- c(
    "WT_0", "osrgf1-8_0", "osrgf1-8_1",
    "osrgf1-8_10", "osrgf1-8_100", "osrgf1-8_1000"
)

df1 <- df0 %>% 
    dplyr::filter(
        genotype %in% c("WT", "osrgf1-8")
    )

df1$genotype <- factor(df1$genotype, levels = c("WT", "osrgf1-8"))
...
cld <- cld %>% 
    dplyr::mutate(
        YPOS_MIN = MIN - 7,
        YPOS_MAX = estimate_cld_pos(MAX) + 4,
        genotype = str_replace(GROUP, "(.*)_(.*)", "\\1")
    )
...
ggplot(df1, aes(group, root_length, color = genotype)) +
    ...
    scale_color_manual(
        values = c("#000000", "#E69F00", "#0072B2", "#CC79A7"),
        labels = c("WT", "<i>osrgf1-8</i>"),
        guide = guide_legend(
            order = 1
        )
    ) +
    # hline_grob(1 - .3, 2 + .3, 0 - 1.4) +
    # hline_grob(3 - .3, 4 + .3, 0 - 1.4) +
    # hline_grob(5 - .3, 6 + .3, 0 - 1.4) +
    # hline_grob(7 - .3, 8 + .3, 0 - 1.4) +
    # hline_grob(1 - .3, 4 + .3, 0 - 8.5) +
    # hline_grob(5 - .3, 8 + .3, 0 - 5.1) +
