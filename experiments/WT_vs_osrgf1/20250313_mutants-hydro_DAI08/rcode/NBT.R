...
df0 <- rawdata %>% 
    ...
    # dplyr::mutate(
    #     is_outlier = is_outlier(nbt_area, use_median = TRUE)
    # ) %>% 
    # dplyr::ungroup() %>% 
    # dplyr::filter(!is_outlier) %>% 
    dplyr::mutate(genotype = factor(genotype, levels = c("WT", "M07", "M08", "M15")))


stats_res <- oneway_test(df0, nbt_area ~ genotype)
stats_res$tests
stats_res <- stats_res$result
stats_res

ggplot(df0, aes(genotype, nbt_area, colour = genotype)) +
    labs(
        subtitle = "Meristematic zone size",
        y = "NBT staining area (pixel)"
    ) +
    geom_boxplot() +
    geom_point() +
    geom_text(
