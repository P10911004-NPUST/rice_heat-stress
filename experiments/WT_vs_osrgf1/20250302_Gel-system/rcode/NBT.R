...
write.csv(df0, "./tables/NBT.csv", row.names = FALSE)

#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
# EdU avg intensity ====
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
stats_res <- oneway_test(df0, nbt_intensity ~ genotype)
stats_res$tests
stats_res <- stats_res$result
stats_res

ggplot(df0, aes(genotype, nbt_intensity, color = genotype)) +
    labs(
        subtitle = "Superoxide level",
