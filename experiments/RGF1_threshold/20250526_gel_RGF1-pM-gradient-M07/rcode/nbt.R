...
df0 <- rawdata %>% 
    ...
    dplyr::mutate(
        ...
        nbt_intensity = nbt_area * trim_avg_nbt / 1e6
    )
...
out <- oneway_test(df_seminal, nbt_intensity ~ treatment)
out
cld <- out$cld
cld$cld_pos <- estimate_cld_pos(cld$MAX)

ggplot(df_seminal, aes(treatment, nbt_intensity, color = treatment)) +
    theme_bw() +
    labs(
        subtitle = "Gel system, DAI10, <i>osrgf1-7</i>, seminal root",
        x = "OsRGF1 (pM)",
        y = "Total NBT intensity"
    ) +
...
out <- oneway_test(df_crown, nbt_intensity ~ treatment)
out
cld <- out$cld
cld$cld_pos <- estimate_cld_pos(cld$MAX)

ggplot(df_crown, aes(treatment, nbt_intensity, color = treatment)) +
    theme_bw() +
    labs(
