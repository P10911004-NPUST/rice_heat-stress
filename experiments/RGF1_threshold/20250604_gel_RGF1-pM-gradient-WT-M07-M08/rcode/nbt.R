...
out <- oneway_test(df1, nbt_intensity ~ treatment)
out$tests
cld <- out$cld
cld$cld_pos <- estimate_cld_pos(cld$MAX)

ggplot(df1, aes(treatment, nbt_intensity, color = treatment)) +
    theme_bw() +
    labs(
...
ggsave(
    filename = "nbt_intensity_M07.jpg",
    device = "jpeg",
    dpi = 660,
    units = "cm",
    height = 11,
...
ggsave(
    filename = "nbt_intensity_M08.jpg",
    device = "jpeg",
    dpi = 660,
    units = "cm",
    height = 11,
...
out <- oneway_test(df1, nbt_intensity ~ group)
out$tests
out$pre_hoc
cld <- out$cld
cld$cld_pos <- estimate_cld_pos(cld$MAX)

ggplot(df1, aes(group, nbt_intensity, color = treatment)) +
    theme_classic() +
    labs(
    ...
    hline_grob(1 - .3, 2 + .3, .4) +
    hline_grob(3 - .3, 4 + .3, .4) +
    theme(
        text = element_text(family = "sans", face = "bold", size = 22),
...
ggsave(
    filename = "nbt_intensity_WT-M08.jpg",
    device = "jpeg",
    dpi = 660,
    units = "cm",
    height = 11,
