...
df0 <- rawdata %>% 
    ...
    dplyr::mutate(
        ...
        heat = factor(paste0("T", heat), levels = c("T28", "T38")),
...
df0$group <- factor(
    df0$group, 
    levels = c("WT_T28_0", "osrgf1-7_T28_0", "WT_T38_0", "osrgf1-7_T38_0",
               "WT_T28_1", "osrgf1-7_T28_1", "WT_T38_1", "osrgf1-7_T38_1")
)
...
for (g in unique(df0$group)) {
    ...
    print(paste0(g, ": ", is_normality(df_tmp$nbt_total)))
}
...
ggplot(df0, aes(group, nbt_total, color = heat)) +
    ...
    ggsignif::geom_signif(
        test = "t.test",
        comparisons = list(
            c("WT_T28_0", "osrgf1-7_T28_0"),
            c("WT_T38_0", "osrgf1-7_T38_0"),
            c("WT_T28_1", "osrgf1-7_T28_1"),
            c("WT_T38_1", "osrgf1-7_T38_1")
        ),
        y_position = c(30, 22, 39, 39),
        tip_length = c(
            0.05, 0.05,
            0.15, 0.05,
            0.05, 0.05,
    ...
    geom_text(
        inherit.aes = FALSE,
        data = cld,
        mapping = aes(GROUP, YPOS_MIN, label = N, color = heat),
        size = 7
    ) +
    ...
    hline_grob(1 - 0.3, 4 + 0.3, 0 - 2) +
    hline_grob(5 - 0.3, 8 + 0.3, 0 - 2) +
    theme(
