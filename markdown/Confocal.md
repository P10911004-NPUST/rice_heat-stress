...
> -   若 monitor 熒幕出現 "lower Z limit" 訊息，這表示載物台已處於最低位置。 執行其他操作前，先把載物台略微上調 ( 載物台滾輪 順時鐘 旋轉 ), 該訊息會自動消失, monitor 才可正常操作。
...
5.  先用目鏡找樣本並對焦;
6.  回到 Acquisition 巨集，使用主波長，調整焦距;
7.  後續步驟和一般流程一樣;
8.  換玻片必須點 Load position;
9.  使用完油鏡後，按 Load position, 卸下玻片, 以 Lens paper 和 100% 酒精, 輕緩擦拭油鏡鏡頭;
